...
export const pomoReducer = (state = initialState, action) => {
  switch (action.type) {
    case actions.COMPLETE_POMO:
      ...
      if (index === -1)
        throw new Error("Somehow can't find activity", action.activity);

      // build new state
      const incrementedActivity = Object.assign(
        {},
        { name: action.activity },
        { completedSessions: state.activities[index].completedSessions + 1 }
      );
    ...
    case actions.REMOVE_ACTIVITY:
      const newActivitiesArray = state.activities.filter(
        activity => activity.name !== action.activity
      );
