...
describe("Smoke tests", () => {
  ...
  it("Activity component renders without crashing", () => {
    const wrapper = shallow(
      <Activity deleteActivity={() => {}} onClick={() => {}} />
    );
...
describe("Shallow Components", () => {
  describe("Circular progress bar", () => {
    it("renders with props", () => {
      ...
      const renderer = TestUtils.createRenderer();
      renderer.render(
        <CircularProgress totalSeconds={totalSeconds} percentage={percentage} />
      );
      const result = renderer.getRenderOutput();
  ...
  describe("Article", () => {
    it("renders proper image if length is 5", () => {
      ...
      result.props.children[0].props.children.props.src.should.equal(
        props.data.multimedia[4].url
      );
    });
